    ...
    if (data instanceof Array) {
        ...
        for (let i = 0; i < data.length; i++) {
            for (k in data[i]) {
                if (isChangeTimeForZone(k, data[i][k])) {
                    data[i][k] = changeTimeByZone(req, k, data[i][k])
                } else {
        ...
        for (k in data) {
            if (isChangeTimeForZone(k, data[k])) {
                data[k] = changeTimeByZone(req, k, data[k])
            } else {
...
function isChangeTimeForZone(k, v) {
    let dateName = ["createdAt", "updatedAt"]
    if (dateName.indexOf(k) > -1) {
        return true
    }

    //如果key是以time结束，并且值是，并且类型为Time，就进行转换
    if (String(k).match(/At$/) || String(k).match(/Time$/) || String(k).match(/Date$/)) {
 ...
 * @returns
 */
function changeTimeByZone(req, k, data) {
    let format = String(k).match(/Date$/) ? "MM/DD/YYYY" : "MM/DD/YYYY HH:mm:ss"
